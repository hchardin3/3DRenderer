...
Camera::Camera(Eigen::Vector3d position, double horizontalFOV, double verticalFOV,  int horizontalResolution,  int verticalResolution,  double distance) :
...
Eigen::Vector3d Camera::getPositionPixel(const int i, const int j) const {
    Eigen::Vector3d pixelPosition = m_position + getForward() * m_distance;
